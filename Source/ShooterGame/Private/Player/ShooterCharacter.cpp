...
{
	if (!GetCharacterMovement() || stuned)
	{
		return false;
	}
...
void AShooterCharacter::JetPack_Server_Implementation(FVector Pos)
{
...
{
	if (teleportCD <= 0 && !stuned)
	{
		/* Addapt Unreal distance to Meters */
		int constantMult = 100;
		
		/* LineTrace Hit */
		FHitResult Hit;

		/* FVectors LineTrace */
		FVector OffSet = Mesh1P->GetRightVector() * constantMult;
		FVector Foward = Mesh1P->GetRightVector();
		FVector Start = GetWeapon()->GetActorLocation();
		FVector End = Start + (Foward * constantMult * (distance - 1));

		//Verify Collisions with Static Objects
		FCollisionQueryParams ColParam;
		ColParam.AddIgnoredActor(this);
		bool isHit = GetWorld()->LineTraceSingleByChannel(Hit, Start, End, ECC_WorldStatic, ColParam);

		/* Draw To Help Debug, Delete or Comment Later */
		DrawDebugLine(GetWorld(), Start, End, FColor::Red, false, 1, 0, 1);

		#pragma region Set New Position

		if (isHit)
		{
			/* LineTrace Hit -  CollisionNormal * CapsuleSize */
			FVector newEnd = Hit.ImpactPoint;


			if (FVector::Distance(GetActorLocation(), newEnd) > 100)
			{
				//TODO: Create a ToServer and ToNetMulticast SetActorLocation();
				SetActorLocation(newEnd);
			}
		}
		else
		{
		...
		/* Reset Teleport CD */
		teleportCD = teleportCDDefault;

		#pragma endregion
	}	
}

void AShooterCharacter::TeleportTimer()
{
	if (teleportCD > 0)
	{
		teleportCD -= GetWorld()->GetDeltaSeconds();
	}
}
...
void AShooterCharacter::GetLifetimeReplicatedProps(TArray< FLifetimeProperty > & OutLifetimeProps) const
{
	...
	/* Only Owner, Server confirmation to prevent usage of third party programs */
	DOREPLIFETIME_CONDITION(AShooterCharacter, stuned, COND_OwnerOnly);
	DOREPLIFETIME_CONDITION(AShooterCharacter, currentFuel, COND_OwnerOnly);
	DOREPLIFETIME_CONDITION(AShooterCharacter, maxFuel, COND_OwnerOnly);
	DOREPLIFETIME_CONDITION(AShooterCharacter, impulseForce, COND_OwnerOnly);
	DOREPLIFETIME_CONDITION(AShooterCharacter, distance, COND_OwnerOnly);
	DOREPLIFETIME_CONDITION(AShooterCharacter, teleportCD, COND_OwnerOnly);
	DOREPLIFETIME_CONDITION(AShooterCharacter, teleportCDDefault, COND_OwnerOnly);
}
